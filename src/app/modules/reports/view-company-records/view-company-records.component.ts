...
export class ViewCompanyRecordsComponent implements OnInit, OnDestroy {
  checkCompany = true;
        ...
        this.http
          ...
          .subscribe(
            (response) => {
              ...
              for (var x in this.fetchedData) {
                this.fetchedData[x].DOCUMENT_NAME =
                  this.fetchedData[x].DOCUMENT_NAME.split(' ').join('%20');
                this.fetchedData[x].DOCUMENT_NAME =
        ...
        .subscribe(
          (response) => {
            ...
            if (this.internalDocs[0].response.length <= 0) {
              ...
              var newstringDoc = this.fileResponse
                .split('=')
                .pop()
                .split('>')[0];
              var newstringDocName = this.fileResponse
                .split('=')
                .pop()
                .split('>')[1];

              var newstringDocLink =
      ...
      this.http
        ...
        .subscribe((response) => {
          ...
          if (this.notingResponse[0].response.length <= 0) {
            ...
            this.spinnerLoading2 = false;
            this.notingResponseStatus = false;
            var newNoting = this.notingResponse[0].response
              .split('<<')
              .join('')
              .split('>>');
